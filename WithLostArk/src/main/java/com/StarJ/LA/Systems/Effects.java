	...
	private static class Toast {
		...
		public Toast(Material icon, String msg, AdvancementFrameType frame) {
			...
			this.display = new AdvancementDisplay(CraftItemStack.asNMSCopy(new ItemStack(icon)),
					IChatBaseComponent.a(msg), IChatBaseComponent.a(" "), new MinecraftKey("minecraft", background),
		...
		public void sendMessage(Player player) {
			...
			Advancement ad = getAdvancement();
			net.minecraft.advancements.AdvancementProgress progress = p.M().b(ad);
			if (!progress.a())
				p.M().a(ad, getMinecraftkey().toString());
			try {
				new BukkitRunnable() {
					@Override
					public void run() {
						p.M().b(ad, getMinecraftkey().toString());
					}
