...
public class BlitzRush extends Skills {
		...
		if (ComboCoolRunnable.hasCombo(player, this)) {
			...
			new BukkitRunnable() {
				...
				public void run() {
					...
					if (hit)
						BuffRunnable.run(player, BlitzRush.this, getDuration(), slot);
			...
			new BukkitRunnable() {
				...
				public void run() {
					...
					player.setVelocity(new Vector());
					ComboCoolRunnable.run(player, BlitzRush.this, getComboDuration(), slot);
					player.playSound(player, Sound.ENTITY_PLAYER_ATTACK_SWEEP, 3f, 1f);
				}
