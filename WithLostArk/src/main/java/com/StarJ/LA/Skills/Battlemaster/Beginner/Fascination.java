				...
				public void run() {
					if (off.isOnline()) {
						...
						if (this.time < 5 && ConfigStore.getJob(player).getKey().equals(job)) {
							boolean has = HashMapStore.hasAttackList(att, Fascination.this);
							if (has)
								HashMapStore.removeAttackList(att, Fascination.this);
							damage(att, vic, getDamage(att));
							if (has)
								HashMapStore.addAttackList(att, Fascination.this);
